...
if (!$lab_object) {
    ...
    foreach ($properties_list as $property_object) {
        ...
        $papers[$id]['start'] = $property_object->get_display_start_time();
    ...
    if (count($papers) > 0) {
        $render->render($papers, $lang, 'invigilator/header.html');
        $render->render($papers, $lang, 'invigilator/paper.html');
    } else {
        $data['emergency'] = $invigilation->emergencyNumbers();
        $render->render($data, $lang, 'invigilator/nopapers.html');
    }
    $mysqli->close();
