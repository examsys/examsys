...
if (isset($_GET['paperID'])) {
  ...
  $form_html = $string['screen'] . "&nbsp;<select name=\"screen\">\n";
  $max_screen = $_GET['max_screen'];
  for ($i=1; $i<=$max_screen + 1; $i++) {
    if ($i == $max_screen) {
      $form_html .= "<option value=\"$i\" selected>$i</option>\n";
    } else {
  ...
  $form_html = '<input type="hidden" name="module" value="' . $_GET['module'] . '" /><input type="hidden" name="session" value="' . $_GET['session'] . '" />';
  if (isset($_GET['objectives'])) {
    $form_html .= '<input type="hidden" name="objectives" value="' . $_GET['objectives'] . '" />';
  }
