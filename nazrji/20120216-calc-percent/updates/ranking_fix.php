...
while ($result->fetch()) {
  ...
  while ($result2->fetch()) {
    ...
    //var_dump($correct, $answers, $mark);
    
    //if ($mark != $original_mark) {
      echo "<div>$score_method, UPDATE log2 SET mark=$mark, totalpos=$totalpos WHERE id=$log_id</div>";
      
      
      $adjust = $mysqli->prepare("UPDATE log2 SET mark=?, totalpos=? WHERE id=?");
