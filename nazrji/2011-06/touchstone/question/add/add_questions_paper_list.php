...
<?php
  ...
  if (isset($_GET['paper_type'])) {
    $sql = "SELECT property_id, paper_title, paper_type, moduleID, DATE_FORMAT(created,'%d/%m/%y') AS created, title, initials, surname FROM (properties, users) WHERE paper_type='" . $_GET['paper_type'] . "' AND deleted IS NULL AND paper_ownerID=users.id AND (paper_ownerID=$userID $my_teams) ORDER BY paper_title";
  } else {
    $sql = "SELECT property_id, paper_title, paper_type, moduleID, DATE_FORMAT(created,'%d/%m/%y') AS created, title, initials, surname FROM (properties, users) WHERE moduleID LIKE '%" . $_GET['team_name'] . "%' AND deleted IS NULL AND paper_ownerID=users.id ORDER BY paper_title";
  }
