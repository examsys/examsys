...
Class Question extends RogoObject {
  ...
  protected $_user_id;
  protected $_fields = array('type', 'theme', 'scenario', 'scenario_plain', 'leadin', 'leadin_plain', 'notes', 'correct_fback', 'incorrect_fback', 'score_method', 'display_method', 'option_order', 'standards_setting', 'bloom', 'owner_id', 'media', 'media_width', 'media_height', 'checkout_time', 'checkout_author_id', 'created', 'last_edited', 'locked', 'deleted', 'status', 'draft');
...
//  protected $_score_methods = array('Mark per Question', 'Mark per Option', 'Allow partial Marks', 'Bonus Mark');
  ...
  function __construct($mysqli, $user_id, $lang_strings, $data = null) {
    ...
    if (is_array($data)) {
      ...
      $this->id = $data;
      if (!$this->get_question($question_mode)) {
...
creation_date, last_edited, locked, deleted, status, draft)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
QUERY;
      } else {
...
checkout_time = ?, checkout_authorID = ?, creation_date = ?, last_edited = ?, locked = ?, deleted = ?, status = ?, draft = ?
WHERE q_id = ?
QUERY;
      }
...
QUERY;
  ...
  /**
   * Get the draft time of the question
   * @return string
   */
  ...
  /**
   * Set the draft time of the question
   * @param string $value
   */
...
QUERY;
   ...
   */
  public static function question_factory($mysqli, $user_id, &$lang_strings, $data, $question_mode='live') {
    $object = null;
...
QUERY;
      ...
      if ($result->fetch()) {
        ...
        try {
          include $classfile;
          $object = new $classname($mysqli, $user_id, $lang_strings, $data, $question_mode);
        } catch (Exception $ex) {
      ...
      try {
        include $classfile;
        $object = new $classname($mysqli, $user_id, $lang_strings, null, $question_mode);
      } catch (Exception $ex) {
        throw new ClassNotFoundException(sprintf($lang_strings['noclasserror'], $classname));
      }
    }
   ...
   */
  private function get_question($question_mode = 'live') {
    // Get the question
    $found = 0;
    $success = false;
...
SELECT q_type, theme, scenario, scenario_plain, leadin, leadin_plain, notes, correct_fback, incorrect_fback, score_method, display_method,
 ...
 last_edited, locked, deleted, status, draft
FROM questions
WHERE q_id = ?
QUERY;
    if ($question_mode != 'live') {
      $q_query .= ' AND draft IS NOT NULL';
    }
...
QUERY;
      ...
      if ($question_mode != 'live') {
        // No draft version so get the live version
